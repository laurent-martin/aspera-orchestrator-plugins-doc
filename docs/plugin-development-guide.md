...
This guide explains how to develop a plugin (action) for Aspera Orchestrator.
Plugins allow you to extend Orchestrator's functionality by adding custom actions that can be used in workflows.

### What is a Plugin?

A plugin (also called an "action") is a Ruby class that extends Orchestrator's capabilities.
...
1. **Discovery**: Plugins are stored in the `actions/` directory (configurable via config parameter `actionplugins_load_directory`)
...
| File Type | Usage | Purpose | Example |
|-----------|-------|---------|---------|
| **Controllers** | ~21 plugins | Complex UI interactions and AJAX operations | `akamai_transcoding_controller.rb` |
| **Helpers** | ~8 plugins | View helper methods for formatting data and generating UI elements | `ateme_transcoding_helper.rb` |
| **Services** | ~5 plugins | Encapsulate business logic and external API calls | `faspex5_package_monitor_authentication_service.rb` |

### File Naming Conventions

| File Type | Naming Convention | Example |
|-----------|-------------------|---------|
| **Main class file** | Must match the class name in snake_case | `HelloWorld` → `hello_world.rb` |
| **Migration files** | Format `YYYYMMDDHHMMSS_description.rb` (timestamp ensures execution order) | `20231201120000_create_table.rb` |
| **Icon file** | Must match the class name | `HelloWorld.png` |
| **View files** | `edit.html.erb` for configuration, `help.html.erb` for documentation | `edit.html.erb`, `help.html.erb` |

### Directory Location

Plugins are installed in the directory specified by parameter `actionplugins_load_directory`, which defaults to `config/orchestrator/actions`.
During development, plugins are typically placed in the `actions/` directory at the Orchestrator application root.

## Plugin Architecture Patterns

Plugins follow the following patterns and mechanisms:
...
- Define constants for all input/output variable names
...
| `inputs_spec` | Returns two `Hash`es: `required_inputs` and `optional_inputs` |
...
| Pattern | Configuration | Behavior | Examples |
|---------|--------------|----------|----------|
| **Synchronous Execution** (default) | Default settings | `execute` method completes the work and returns final status: `[status, status_details, outputs]` | Simple file operations, API calls, data transformations |
| **Asynchronous Execution** | Override `synchronous_execution?` to return `false` | `execute` starts the operation and returns `STATUS_INPROGRESS`. Implement `check_status` to poll for completion | Transcoding jobs, large file transfers, batch processing |
| **Trigger Pattern** | Set `synchronous_execution?` to `false` and `multiple_execution?` to `true` | Use `check_status` to detect events and return `STATUS_COMPLETE` when triggered. Include `ModuleTriggerTools` module for trigger persistence | File monitoring, package arrival detection, webhook listeners |
...
def self.version
  ...
  versions = SafeYaml.load_file(File.join(__dir__,'metadata.ym'))[:revision_history].map { |change| change[:version].split(".").map { |num| num.to_i } }
...
def self.display_name
  ...
  @@display_name = SafeYaml.load_file(File.join(__dir__,'metadata.ym'))[:display_name]
end
```

> [!NOTE]
> If not defined, the system generates a name from the class name (e.g., `MyPlugin` &rarr; "My plugin").

#### `name` (Optional)

Provides a human-readable identifier for a configured instance.

Used by ~3 plugins for dynamic naming.
...
CATEGORY_ASSET
CATEGORY_FILEOPERATIONS
CATEGORY_FILETRANSFER
...
CATEGORY_RESOURCE
CATEGORY_SYSTEM
CATEGORY_TRANSCODING
CATEGORY_TRIGGERS
CATEGORY_USERINTERACTIONS
CATEGORY_VIRUS
...
| Plugin Type | Purpose | Execution | Examples | Best For | Key Features |
|-------------|---------|-----------|----------|----------|--------------|
| **Standard Action Plugins** | Execute specific tasks within a workflow | Synchronous or asynchronous | File operations, API calls, data transformations | One-time operations with clear inputs and outputs | - |
| **Trigger Plugins** | Monitor for events and spawn new workflow instances | Always asynchronous with `multiple_execution?` = true | File system monitors, message queue listeners | Event-driven workflows, continuous monitoring | Include `ModuleTriggerTools` for trigger persistence |
| **Integration Plugins** | Connect to external systems and services | Varies | SOAP/REST APIs, databases, cloud services | System integration, data exchange | Use `Gemfile` for dependencies, store WSDL files, use payload templates |
| **Custom Trigger with Code Evaluation** | Maximum flexibility for custom logic | User-defined | Custom Ruby code execution | Power users who need complete control | Allows custom Ruby code for inputs, outputs, validation, and execution. **Warning**: Security implications |
...
- Versions are defined in key `:revision_history` in `metadata.yml`
...
Place in `actions/` directory and reload
